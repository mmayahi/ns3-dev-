...
#ifndef TWT_RR_MULTI_USER_SCHEDULER_H
#define TWT_RR_MULTI_USER_SCHEDULER_H
 ...
 * 
 * TwtRrMultiUserScheduler is modified from RrMultiUserScheduler. 
 *
 */
class TwtRrMultiUserScheduler : public MultiUserScheduler
{
public:
  /**
  ...
  TwtRrMultiUserScheduler ();
  virtual ~TwtRrMultiUserScheduler ();
...
private:
  ...
  /**
   * Create Dl Mu info for TWT STAs forcefully without sending or queueing packets
   *
   * \return DL_MU_TX if it is possible to create dlMuInfo or NO_TX otherwise
   */
  virtual TxFormat ForceCreateDlMuInfo (void);
  
  /**
   * Create Dl Mu info for TWT STAs WITH KNOWN NON-EMPTY BUFFER STATUS ONLY forcefully without sending or queueing packets
   *
   * \return DL_MU_TX if it is possible to create dlMuInfo or NO_TX otherwise
   */
  virtual TxFormat ForceCreateDlMuInfoKnownStaOnly (void);
  /**
  ...
  struct MasterInfo
  {
    ...
    uint64_t queueSize;               //!< Last known queue size for this STA TID queue
  };
...
#endif /* TWT_RR_MULTI_USER_SCHEDULER_H */
