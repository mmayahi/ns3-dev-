...
void
FrameExchangeManager::SetTwtStaToSleep (void)
{
  NS_LOG_FUNCTION (this);
  if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
  {
    Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
    
    staMac->SleepWithinTwtSpNow();
  }
  return;
}

void
FrameExchangeManager::NotifyStaNormalOrBlockAckTransmitted (void)
{
  NS_LOG_FUNCTION (this); 
  if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
  {
    ...
    staMac->NormalOrBlockAckTransmitted();
  }
  m_pendingAckTx = false;
  return;
}
...
FrameExchangeManager::StartTransmission (Ptr<Txop> dcf)
{
  ...
  NS_ASSERT (mpdu != 0);
  NS_ASSERT (mpdu->GetHeader ().IsData () || mpdu->GetHeader ().IsMgt () || mpdu->GetHeader ().IsPsPoll() );
  ...
  NS_LOG_DEBUG ("Wifi Ack method:"<<txParams.m_acknowledgment->method);
  return true;
}
...
FrameExchangeManager::SendMpdu (void)
{
  ...
  // transmit the MPDU
  if (m_mac->GetTypeOfStation() == TypeOfStation::STA && m_mpdu->GetHeader().IsPsPoll())
  {
    // If mpdu is PS POLL for STA, send only if m_PsWaitingForPolledFrame is true for STA
    Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
    if (staMac->isStaPsWaitingForPolledFrame())
    {
      ForwardMpduDown (m_mpdu, m_txParams.m_txVector);
      Time timeout = txDuration
                    + m_phy->GetSifs ()
                    + 5* m_phy->GetSlot ();
      // #fixAsap - hardcoded TODO
      NS_LOG_DEBUG ("STA will check isStaPsWaitingForPolledFrame() after "<<timeout);
      Simulator::Schedule (timeout, &FrameExchangeManager::TrySendPsPollAgain, this);
    }
  }
  else
  {
...
void
FrameExchangeManager::TrySendPsPollAgain ()
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_mac->GetTypeOfStation() == TypeOfStation::STA);
  NS_LOG_DEBUG ("Queuing PS POLL packet into STA MAC again if required");
  Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
  staMac->CheckPsPolledFrameReceived();
}
                                     ...
                                     double dataSnr)
{
  ...
  NS_LOG_DEBUG(" GetAckTxVector:: GetStaId (to, dataTxVector) "<< hdr.GetAddr2 () );
  ...
  Time ackDuration =m_phy->CalculateTxDuration (GetAckSize (), ackTxVector, m_phy->GetPhyBand ());
  NS_LOG_DEBUG ("Scheduling StaWifiMac::NormalOrBlockAckTransmitted after the Normal Ack frame duration if it is an STA");
  Simulator::Schedule (ackDuration, &FrameExchangeManager::NotifyStaNormalOrBlockAckTransmitted, this);
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG ("FrameExchangeManager::TransmissionSucceeded");
  ...
  if (m_moreFragments)
  ...
  else
    {
      ...
      if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
      {
        NS_LOG_DEBUG ("This is a non-QoS STA. Transmission of last frame succeeded.");
        Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
        // NS_LOG_DEBUG ("staMac is at: "<<staMac);
        staMac->SleepAfterAckIfRequired ();
...
FrameExchangeManager::NormalAckTimeout (Ptr<WifiMacQueueItem> mpdu, const WifiTxVector& txVector)
{
  ...
  // Shyam - makeshift solution for failed TWT Setup frames; If this is a TWT Setup frame, keep retrying
  const WifiMacHeader& hdr = mpdu->GetHeader ();
  if (hdr.IsMgt ()  && hdr.IsAction () && (m_mac->GetTypeOfStation() == TypeOfStation::AP)) 
  {
    WifiActionHeader actionHdr;
    Ptr<Packet> packet = mpdu->GetPacket ()->Copy ();
    packet->RemoveHeader (actionHdr);
    if (actionHdr.GetCategory () == WifiActionHeader::UNPROTECTED_S1G && actionHdr.GetAction().unprotectedS1GAction == WifiActionHeader::TWT_SETUP)
    {
      NS_LOG_DEBUG ("This is a TWT Setup frame. Keep retrying");
                               ...
                               WifiTxVector txVector, std::vector<bool> perMpduStatus)
{
  ...
  if (!addr1.IsGroup () && addr1 != m_self)
    {
      NS_LOG_DEBUG ("ignoring unicast frames that are not addressed to us.");
      // if this is a HE STA, call StaWifiMac::ReceivedUnicastNotForMe
      if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
        {
          NS_LOG_DEBUG ("This is an STA. Calling StaWifiMac::ReceivedUnicastNotForMe");
          Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
          staMac->ReceivedUnicastNotForMe ();
        }
  ...
  if (psdu->GetNMpdus () == 1)
    {
      ...
      Ptr<WifiMacQueueItem> tempmpdu = *(psdu->begin ());
      NS_LOG_DEBUG ("Calling ReceiveMpdu with values *tempmpdu:"<<*tempmpdu);
      // NS_LOG_DEBUG ("Calling ReceiveMpdu");
      ReceiveMpdu (*(psdu->begin ()), rxSignalInfo, txVector, perMpduStatus.empty ());
      NS_LOG_DEBUG ("Called ReceiveMpdu");
    }
  else
    {
      // NS_LOG_DEBUG ("Skipped the if block");
...
{
  NS_LOG_DEBUG ("Entered ReceiveMpdu");
  ...
  if (hdr.IsCtl ())
    {
      NS_LOG_DEBUG ("Received MPDU is CTL");
      if (hdr.IsRts ())
        {
        ...
        {
          NS_ASSERT (hdr.GetAddr1 () == m_self);
          NS_LOG_DEBUG ("Received ACK while waiting for ACK");
          SnrTag tag;
          mpdu->GetPacket ()->PeekPacketTag (tag);
        ...
        {
          NS_ASSERT (hdr.GetAddr1 () == m_self);
          Mac48Address sender = hdr.GetAddr1 ();
          m_mac->GetWifiRemoteStationManager ()->ReportRxOk (sender, rxSignalInfo, txVector);
          NS_LOG_DEBUG ("Received PS-POLL from=" << hdr.GetAddr2 ()<<";");
//          Simulator::Schedule (m_phy->GetSifs (), &FrameExchangeManager::SendNormalAck,
//                               this, hdr, txVector, rxSnr);
          // m_mpdu = 0;
          // Sending PS-POLL to mac middle
          m_rxMiddle->Receive (mpdu);
        }
      else if (hdr.IsTrigger ())
        {
          NS_LOG_DEBUG ("Received MPDU is trigger");
          // NS_ASSERT (hdr.GetAddr1 () == m_self);
          Mac48Address sender = hdr.GetAddr1 ();
          m_mac->GetWifiRemoteStationManager ()->ReportRxOk (sender, rxSignalInfo, txVector);
          NS_LOG_DEBUG ("Received Trigger frame");
          
          // Sending Trigger frame to mac middle
          // m_rxMiddle->Receive (mpdu);
        }
    }
  else if (hdr.IsMgt ())
...
void 
FrameExchangeManager::SetPendingAckTx (bool pendingAckTx)
{
  NS_LOG_FUNCTION (this << pendingAckTx);
  m_pendingAckTx = pendingAckTx;
}

bool FrameExchangeManager::IsAckTxPending (void) const
{
  NS_LOG_FUNCTION (this);
  return m_pendingAckTx;
}
...
FrameExchangeManager::NotifyReceivedNormalAck (Ptr<WifiMacQueueItem> mpdu)
{
  ...
  // If Node is an STA with TWT, if this MPDU is TWT resume/suspend, change state at STA
  // if (m_mac->GetTypeOfStation() == TypeOfStation::STA)
  // {
  //   Ptr<StaWifiMac> staMac = DynamicCast<StaWifiMac> (m_mac);
  //   if (staMac->GetTwtAgreement())
  //   {
  //     if (mpdu->GetHeader().IsAction())
  //     {
  //       WifiActionHeader actionHdr;
  //       Ptr<Packet> tempPkt;
  //       tempPkt=mpdu->GetPacket()->Copy();
  //       tempPkt->RemoveHeader(actionHdr);
  //       if (actionHdr.GetCategory () == WifiActionHeader::UNPROTECTED_S1G)
  //       {
  //         MgtUnprotectedTwtInfoHeader s1gHdr;
  //         tempPkt->RemoveHeader(s1gHdr);
  ...
  //         staMac->ResumeSuspendTwtIfRequired (twtId, nextTwt);
