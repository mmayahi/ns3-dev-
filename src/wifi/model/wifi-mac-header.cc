...
void WifiMacHeader::SetPowerMgt (void)
{
  m_ctrlPowerMgt = 1;
}

void WifiMacHeader::SetNoPowerMgt (void)
{
  m_ctrlPowerMgt = 0;
}
...
{
  if (GetType () == WIFI_MAC_CTL_PSPOLL)
  {
    return true;
  }
  else
...
bool
WifiMacHeader::IsPowerMgt (void) const
{
  return (m_ctrlPowerMgt == 1);
}
...
WifiMacHeader::PrintFrameControl (std::ostream &os) const
{
  os << "ToDS=" << std::hex << (int) m_ctrlToDs << ", FromDS=" << std::hex << (int) m_ctrlFromDs
     ...
     << ", PowerManagement=" <<  std::hex << (int) m_ctrlPowerMgt 
