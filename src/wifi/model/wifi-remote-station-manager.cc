...
WifiRemoteStationManager::RecordGotAssocTxFailed (Mac48Address address)
{
  ...
  // If this station was in PS = 1, change its state to PS = 0
  SetPSM (address, false);
  LookupState (address)->m_twtAgreement = false;
}
...
NS_LOG_DEBUG ("GetStaId (Mac48Address address, const WifiTxVector& txVector) :;;; " << address << " ;;; " << txVector << "  ;;;");
...
{                
  //std::cout<<" GetAckTxVector:: GetStaId (to, dataTxVector) "<< GetStaId (to, dataTxVector) <<std::endl;
...
WifiRemoteStationManager::LookupState (Mac48Address address) const
{
  ...
  state->m_PSM = false;
  state->m_expectingBasicTf = false;
  state->m_twtAgreement = false;
...
void
WifiRemoteStationManager::SetPSM (Mac48Address remoteAddress, bool psmEnabled)
{
  NS_LOG_FUNCTION (this << remoteAddress << psmEnabled);
  NS_LOG_DEBUG ("PSM set for STA with MAC "<< remoteAddress <<" from "<<LookupState (remoteAddress)->m_PSM<<" to "<<psmEnabled);
  if (LookupState (remoteAddress)->m_PSM != psmEnabled)
  {
    // STA PS state has changed. Update m_nStaInPs
    NS_LOG_DEBUG ("m_nStaInPs was "<<m_nStaInPs);
    if (psmEnabled)
    {
      m_nStaInPs++;
    }
    ...
    NS_LOG_DEBUG ("m_nStaInPs is now "<<m_nStaInPs);
  }
  LookupState (remoteAddress)->m_PSM = psmEnabled;
  
  return;
}
void
WifiRemoteStationManager::SetExpectingBasicTf (Mac48Address remoteAddress, bool expectingBasicTf)
{
  NS_LOG_FUNCTION (this << remoteAddress << expectingBasicTf);
  NS_LOG_DEBUG ("m_expectingBasicTf changed for STA "<< remoteAddress <<" from "<<LookupState (remoteAddress)->m_expectingBasicTf<<" to "<<expectingBasicTf);
  if (LookupState (remoteAddress)->m_expectingBasicTf != expectingBasicTf)
  {
    // State changed
    NS_LOG_DEBUG ("m_nStaExpectingBasicTf was "<<m_nStaExpectingBasicTf);
    if (expectingBasicTf)
    {
      m_nStaExpectingBasicTf++;
    }
    else
    {
      m_nStaExpectingBasicTf--;
    }
    NS_LOG_DEBUG ("m_nStaExpectingBasicTf is now "<<m_nStaExpectingBasicTf);
  }
  LookupState (remoteAddress)->m_expectingBasicTf = expectingBasicTf;
  
  return;
}

void 
WifiRemoteStationManager::CreateTwtAgreement (uint8_t flowId, Mac48Address peerMacAddress, bool isRequestingNode, bool isImplicitAgreement, bool flowType, bool isTriggerBasedAgreement, bool isIndividualAgreement, u_int16_t twtChannel, Time wakeInterval, Time nominalWakeDuration, Time nextTwt, Time timeLeftTillNextBeacon)
{
  NS_LOG_FUNCTION (this << flowId << peerMacAddress << isRequestingNode << isImplicitAgreement << flowType << isTriggerBasedAgreement << isIndividualAgreement << twtChannel << wakeInterval << nominalWakeDuration << nextTwt << timeLeftTillNextBeacon);
  NS_ASSERT_MSG (IsAssociated(peerMacAddress), "TWT agreement can only be created for associated STAs");
  NS_ASSERT_MSG (flowId < 8 && flowId >= 0, "Flow ID must be between 0 and 7");
  NS_ASSERT_MSG (isRequestingNode == false, "At AP side, isRequestingNode must be false");
  ...
  // first create a WifiTwtAgreement object with given parameters, then add it to the list of agreements for the given STA, if flowId already exists-> replace it with new agreement
  ...
  // Check if agreement already exists for this flowId
  if (LookupState (peerMacAddress)->m_twtAgreementMap.find (flowId) != LookupState (peerMacAddress)->m_twtAgreementMap.end ())
  {
    // Agreement already exists for this flowId. Replace it with new agreement
    NS_LOG_DEBUG ("TWT agreement already exists for flowId "<<(int)flowId<<". Replacing it with new agreement");
    LookupState (peerMacAddress)->m_twtAgreementMap.erase (flowId);
  }

  // schedule next TWT SP at nextTwt
  NS_LOG_DEBUG ("EnableTwtSpNow scheduled at t = "<<(nextTwt + timeLeftTillNextBeacon).GetMilliSeconds()<<" ms");
  // Simulator::Schedule (nextTwt + NanoSeconds (1), &WifiRemoteStationManager::EnableTwtSpNow, this, flowId, peerMacAddress, wakeInterval, nominalWakeDuration);
  Simulator::Schedule (nextTwt + timeLeftTillNextBeacon, &WifiRemoteStationManager::EnableTwtSpNow, this, flowId, peerMacAddress, wakeInterval, nominalWakeDuration);
  
  LookupState (peerMacAddress)->m_twtAgreementMap.insert ({flowId, *agreement});
  LookupState (peerMacAddress)->m_twtAgreement = true;
  if (isTriggerBasedAgreement)
  // TODO: per agreement isTriggerBasedAgreement is not supported yet
  
  {
    NS_LOG_DEBUG ("TWT SP started for TB TWT - setting m_nStaExpectingBasicTf to true. ");
    SetExpectingBasicTf (peerMacAddress, true);
    LookupState (peerMacAddress)->m_twtTriggerBased = isTriggerBasedAgreement;  //TODO - make this per flow
    
  }

  // schedule next TWT SP at nextTwt
  // Simulator::Schedule (nextTwt, &WifiRemoteStationManager::EnableTwtSpNow, this, peerMacAddress, wakeInterval, nominalWakeDuration);
...
// void
// WifiRemoteStationManager::SetTwtParameters (Mac48Address address, u_int8_t twtFlowId, Time targetWakeTime,  Time twtPeriod, Time twtMinWakeDuration, bool twtAnnounced, bool twtTriggerBased)
// WifiRemoteStationManager::SetTwtParameters (Mac48Address address, Time twtPeriod, Time twtMinWakeDuration, bool twtAnnounced, bool twtTriggerBased)
// {
//   NS_LOG_FUNCTION (this << address << twtPeriod << twtMinWakeDuration << twtAnnounced << twtTriggerBased);
//   NS_ASSERT_MSG (LookupState (address)->m_twtAgreement == false, "TWT agreement already exists for this STA. Terminating simulation.");
  
//   LookupState (address)->m_twtAgreement = true;
//   LookupState (address)->m_twtPeriod = twtPeriod;
//   LookupState (address)->m_twtMinWakeDuration = twtMinWakeDuration;
//   LookupState (address)->m_twtAnnounced = twtAnnounced;
//   LookupState (address)->m_twtTriggerBased = twtTriggerBased;
//   LookupState (address)->m_twtSpNow = true;
//   LookupState (address)-> m_twtAwakeForSp= true;
//   LookupState (address)-> m_twtActive= true;


//   if (GetTwtTriggerBased (address))
//   {
//     NS_LOG_DEBUG ("TWT SP started for TB TWT - setting m_nStaExpectingBasicTf to true. ");
//     SetExpectingBasicTf (address, true);
//   }


//   NS_LOG_FUNCTION ("m_twtSpNow is now true ");
//   Simulator::Schedule (twtMinWakeDuration, &WifiRemoteStationManager::DisableTwtSpNow, this, address);
//   Simulator::Schedule (twtPeriod, &WifiRemoteStationManager::EnableTwtSpNow, this,  address, twtPeriod, twtMinWakeDuration);
//   NS_LOG_DEBUG ("TWT setup at AP-StaManager for STA "<<address <<" with twtPeriod = "<<twtPeriod << "; minTwtWakeDuration= "<<twtMinWakeDuration << "; twtAnnounced = "<<twtAnnounced << "; twtTriggerBased = "<<twtTriggerBased);
//   return;
// }

void 
WifiRemoteStationManager::UpdateFromTwtInfoFrame (Mac48Address address, u_int8_t twtId, u_int64_t nextTwt)
{
  NS_LOG_FUNCTION (this << address <<twtId << nextTwt);
  NS_ASSERT_MSG (LookupState (address)->m_twtAgreement == true, "No TWT agreement exists for this STA from which TWT IE was received. Terminating simulation.");
  if (LookupState (address)->m_twtAgreement == true)
  {
    if (nextTwt == 0)
    {
      LookupState (address)->m_twtActive = false;
      LookupState (address)->m_twtAwakeForSp = false;
      NS_LOG_INFO ("m_twtActive for "<<address<<" changed to false and STA is assumed to be in SLEEP now");
    }
    else
    {
      LookupState (address)->m_twtActive = true;
      NS_LOG_INFO ("m_twtActive for "<<address<<" changed to true");
    }
  }
}
...
void
WifiRemoteStationManager::EnableTwtSpNow (uint8_t flowId, Mac48Address address, Time wakeInterval, Time nominalWakeDuration)
{

  NS_LOG_FUNCTION (this <<"EnableTwtSpNow called at AP for flow: " << flowId <<"; for STA: " << address <<"; wakeInterval: " << wakeInterval <<"; nominalWakeDuration:" << nominalWakeDuration);
  LookupState (address)->m_twtSpNow = true;   // TODO - change to per flowId
  // NS_LOG_FUNCTION ("m_twtSpNow is now: "<<LookupState (address)->m_twtSpNow);
  SetTwtAwakeForSp (address, true);

  // check if flowId exists in m_twtAgreementMap for address
  if (LookupState (address)->m_twtAgreementMap.find (flowId) == LookupState (address)->m_twtAgreementMap.end ())
  {
    NS_ABORT_MSG ("No TWT agreement exists for flowId "<<(int)flowId<<" for STA "<<address<<". Terminating simulation.");
  }

  WifiTwtAgreement currentAgrement = LookupState (address)->m_twtAgreementMap.find (flowId)->second; 
  
  if (currentAgrement.IsTriggerBasedAgreement())
  {
    NS_LOG_DEBUG ("TWT SP started for TB TWT - setting SetExpectingBasicTf to true. ");
    SetExpectingBasicTf (address, true);
  }

  // schedule end of current TWT SP
  NS_LOG_DEBUG ("DisableTwtSpNow scheduled at t = "<<nominalWakeDuration.GetSeconds()<<" seconds");
  Simulator::Schedule (nominalWakeDuration, &WifiRemoteStationManager::DisableTwtSpNow, this, flowId, address);

  // schedule next TWT SP at wakeInterval
  NS_LOG_DEBUG ("EnableTwtSpNow scheduled at t = "<<wakeInterval.GetSeconds()<<" seconds");
  Simulator::Schedule (wakeInterval, &WifiRemoteStationManager::EnableTwtSpNow, this, flowId, address, wakeInterval, nominalWakeDuration);
  ...
  // TODO: add support for unannounced TWT
  // For unannounced TWT, switch m_twtAwakeForSp automatically to true at beginning of TWT SP

  // if ( !(GetTwtAnnounced (address)))
  //   {
  //     LookupState (address)-> m_twtAwakeForSp = true;
  //   }
  //   NS_LOG_DEBUG ("At beginning of TWT SP, m_twtAwakeForSp is now: "<<LookupState (address)-> m_twtAwakeForSp);
...
void
WifiRemoteStationManager::DisableTwtSpNow (uint8_t flowId, Mac48Address address)
{
  NS_LOG_FUNCTION (this <<"DisableTwtSpNow called at AP for flow: " << flowId <<"; for STA: " << address);
  SetTwtAwakeForSp (address, true);
  // NS_LOG_FUNCTION (this << flowId << address);
  // NS_LOG_DEBUG ("Setting m_nStaExpectingBasicTf for "<<address<<" to false since DisableTwtSpNow routine was called");
  ...
  SetExpectingBasicTf (address, false); // TODO: check if other agreements are active
  if (LookupState (address)->m_twtSpNow == false)
    {
      NS_LOG_DEBUG ("TWT SP flag for "<<address<<" was already false.");
      LookupState (address)-> m_twtAwakeForSp= false;
      return;
    }
  LookupState (address)->m_twtSpNow = false;
  LookupState (address)-> m_twtAwakeForSp= false;
  NS_LOG_FUNCTION ("m_twtSpNow is now false; m_twtAwakeForSp is now false");
  // Find all frames in AP MAC queue for this STA and move them to PsUnicastBuffer
  NS_ASSERT_MSG (m_wifiMac->GetTypeOfStation () == AP, "DisableTwtSpNow can only be called at AP");
  StaticCast<ApWifiMac> (m_wifiMac)->MoveAllPacketsToPsBufferByAddress(address);
  return;
}

bool
WifiRemoteStationManager::GetPSM (Mac48Address address) const
{
  return LookupState (address)->m_PSM;
}

bool
WifiRemoteStationManager::GetExpectingBasicTf (Mac48Address address) const
{
  return LookupState (address)->m_expectingBasicTf;
}

bool
WifiRemoteStationManager::GetTwtAgreement (Mac48Address address) const
{
  return LookupState (address)->m_twtAgreement;
}

bool
WifiRemoteStationManager::GetTwtTriggerBased (Mac48Address address) const
{
  return LookupState (address)->m_twtTriggerBased;
}

bool
WifiRemoteStationManager::GetTwtAnnounced (Mac48Address address) const
{
  return LookupState (address)->m_twtAnnounced;
}

bool
WifiRemoteStationManager::GetTwtSpNow (Mac48Address address) const
{
  return LookupState (address)->m_twtSpNow;
}

bool
WifiRemoteStationManager::GetTwtAwakeForSp (Mac48Address address) const
{
  return LookupState (address)->m_twtAwakeForSp;
}

void
WifiRemoteStationManager::SetTwtAwakeForSp (Mac48Address address, bool twtAwakeForSp)
{
  NS_LOG_FUNCTION (this<<address<<twtAwakeForSp);
  LookupState (address)->m_twtAwakeForSp = twtAwakeForSp;
  if (!twtAwakeForSp)
  {
    NS_LOG_DEBUG ("Since twtAwakeForSp is false for "<<address<<", SetExpectingBasicTf is now set to false even if STA has packets");
    SetExpectingBasicTf(address, false);
  }
...
uint16_t 
WifiRemoteStationManager::GetNStaExpectingTf (void) const
{
  NS_LOG_FUNCTION (this);
  return m_nStaExpectingBasicTf;
}
