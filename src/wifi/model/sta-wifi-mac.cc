...
StaWifiMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::StaWifiMac")
    ...
    .AddAttribute ("MaxMissedBeacons",
                   "Number of beacons which must be consecutively missed before "
...
StaWifiMac::SetPowerSaveMode (bool enable)
{
  ...
  if (enable)
  {
    ...
    NS_LOG_FUNCTION ("PSM to be enabled. STA will be put to sleep by Frame Exchange Manager(FeMan) after receiving ACK for Network Discovery Protocol(NDP).");
