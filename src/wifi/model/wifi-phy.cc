...
WifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiPhy")
    ...
    .AddTraceSource ("PhyRxPayloadBeginWithPacket",
                     "Trace source indicating the reception of the "
                     "payload of a PPDU has begun, with packet",
                     MakeTraceSourceAccessor (&WifiPhy::m_phyRxPayloadBeginPacketTrace),
                     "ns3::WifiPhy::PhyRxPayloadBeginPacketTracedCallback")
...
WifiPhy::Send (WifiConstPsduMap psdus, WifiTxVector txVector)
{
   ...
   *  - we are idle
   */
  NS_LOG_DEBUG ("m_state->IsStateTx () is "<<m_state->IsStateTx ());
  NS_LOG_DEBUG ("m_state->IsStateSwitching () is "<<m_state->IsStateSwitching ());
  ...
  if (!m_phyTxPsduBeginTrace.IsEmpty ())
    {
      m_phyTxPsduBeginTrace (psdus, txVector, txPowerW, txDuration);
    }
