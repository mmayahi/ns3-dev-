...
PhyEntity::StartReceiveField (WifiPpduField field, Ptr<Event> event)
{
  ...
  if (m_wifiPhy->IsStateSleep())
  {
    NS_LOG_FUNCTION("StartReceiveField called while PHY state went to SLEEP. Function will not proceed. All preamble detection events and PHY events are cleared.");
...
PhyEntity::StartReceivePayload (Ptr<Event> event)
{
  ...
  Time rxDuration = event->GetPpdu ()->GetTxDuration () ; //the actual duration of the PPDU should be considered
  ...
  m_wifiPhy->m_phyRxPayloadBeginPacketTrace (event->GetPpdu (), txVector, rxDuration); //this callback (equivalent to PHY-RXSTART primitive) is triggered only if headers have been correctly decoded and that the mode within is supported
...
{
  NS_LOG_FUNCTION (this << *event);
  // NS_LOG_DEBUG ("m_wifiPhy->m_currentPreambleEvents.size () = "<<m_wifiPhy->m_currentPreambleEvents.size ());
//  NS_ASSERT (m_wifiPhy->m_currentPreambleEvents.size () == 1); //Synched on one after detection period // shyam - commented out to avoid error when using PSM #fixAsap
...
PhyEntity::EndPreambleDetectionPeriod (Ptr<Event> event)
{
  ...
  // shyam - makeshift solution ---------------------------------------------- #fixAsap
  if (m_wifiPhy->m_currentPreambleEvents.empty ())
  {
