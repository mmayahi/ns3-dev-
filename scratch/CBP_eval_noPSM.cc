...
std::string currentLoopIndex_string = "000000";     // This will be updated with input from cmd line and used when creating log files. 
...
double simulationTime = 12;                        /* Simulation time in seconds. */
...
double dataPeriod = 0.2;                       /* Application data period in seconds. */
...
// Enable / disable PSM and sleep state using MAC attribute change - through a function - not directly changing the MAC attribute 
  ...
  // Time sifs = phy->GetSifs();     // shyam - hardcoded number here
  // Time difs = 2 * phy->GetSlot() + phy->GetSifs();     // shyam - hardcoded number here
...
//---------------------------------
  ...
  for (uint32_t ii = 0; ii < otherStaCount; ii++)
  {
    ...
    tempUdpClient.Start (Seconds (0.9 + double(2 * ii)/100));
    // std::cout<<"Server in otherSta "<<ii<<" starts at "<<Seconds (0.9 + double(2 * ii)/100).GetSeconds()<<" seconds\n";
