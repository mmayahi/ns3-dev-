...
uint32_t randSeed = 6000;      // Seed for Random Number Generator
...
// Traffic type and parameters ----------------------------------

std::string traffic = "poisson";   // Poisson traffic 
// std::string traffic = "video";   // Video traffic

// Traffic direction
bool enableUplink = true;

// Poisson traffic parameters
uint32_t poissonPayloadSize = 1500; // in Bytes
uint32_t poissonDataRate;    // Poisson data rate in bps
...
bool enableStateLogs = true;      // Enable state logs if true
...
bool enableAsciiTrace {false};    // Enable ascii traces if true
// Logging ---------------------------------------

bool forcePeriodicTraffic = false;     //if true, poisson is converted to predictable periodic traffic
...
bool twtTriggerBased = true; // Set it to false for contention-based TWT
...
// UDP app timing to start only
// Time AppStartMin = (25 * beaconInterval) + MilliSeconds (3);    // backup
Time AppStartMin = (55 * beaconInterval) + MilliSeconds (3);    // backup
Time AppStartMax = (75 * beaconInterval);                       // backup
// Time AppStartMin = (100 * beaconInterval) + MilliSeconds (3);    // backup
// Time AppStartMax = (101 * beaconInterval);                       // backup
// Time AppStartMax = (131 * beaconInterval);                       // backup
// Time AppStartMin = (700 * beaconInterval) + MilliSeconds (3);    // backup
// Time AppStartMax = (900 * beaconInterval);                       // backup
...
double *timeElapsedForSta_ns_TI, *current_mA_TimesTime_ns_ForSta_TI;    // This is only after keepTrackOfEnergyFrom_S
...
setRxBytesAt10Seconds ()
{
  ...
  if (protocol == "udp")
  {
    ...
    {
      rxBytesAt10Seconds[i] = poissonPayloadSize * DynamicCast<UdpServer> (serverApp.Get (i))->GetReceived ();
      // std::cout<<"At UDP Server "<<i<<" Rx Bytes at t=10s is recorded as: "<<rxBytesAt10Seconds[i]<<"\n";
    } 
  }
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("twtTriggerBased", "true or false",twtTriggerBased);
  cmd.AddValue ("protocol", "Transport protocol: tcp or udp",protocol);
  ...
  cmd.AddValue ("poissonPayloadSize", "The application payload size in bytes", poissonPayloadSize);
...
// LogComponentEnable ("TcpSocket", LogLevel (LOG_PREFIX_TIME | LOG_PREFIX_NODE | LOG_LEVEL_ALL));
  ...
  if (enableTwt)
  {
    dlAckSeqType = "AGGR-MU-BAR";   // Use this for MU TWT
  }
  else
  {
  ...
  // uint32_t poissonPayloadSize = 1000; // in Bytes - must fit in the max TX duration when transmitting at MCS 0 over an RU of 26 tones
  ...
  // std::cout<<"\nConfigured APP data rate (bps): "<<packetCountPerBeaconPeriod * 8.0 * poissonPayloadSize/0.1024<<"\n";
  // uint32_t dataRate = 200 * 8 * 1000; // In bps
  ...
  // double packetIntervalUdp = double(poissonPayloadSize*8)/ double(dataRate);
  ...
  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (poissonPayloadSize));
  ...
  */
  ...
  if (traffic == "poisson")
  {
    poissonDataRate = 1000e6; // Constant 1000 Mbps

  }
  else
  {
    std::cout<<"Traffic type unrecognized";
    return 1;
  }
  ...
  if (enableUplink)  
  {
    ...
    for (std::size_t i = 0; i < nStations; i++)
    {
      ...
      ApplicationContainer tempServerApp = server.Install (p2pServerNode);
      ...
      // Client Apps
      OnOffHelper onoff ("ns3::UdpSocketFactory", (InetSocketAddress (p2pNodeInterfaces.GetAddress (1), port+i)));
      ...
      if (traffic == "poisson")
      {
        ...
        {
          offTimeString = "ns3::ExponentialRandomVariable[Mean="+ offTimeStr1.str() +"]";    // Poisson
        }
        // std::string offTimeString = "ns3::ExponentialRandomVariable[Mean="+ offTimeStr1.str() +"]";
        // std::cout<<"\noffTimeString = "<<offTimeString;
        // onoff.SetAttribute ("OnTime",  StringValue ("ns3::ConstantRandomVariable[Constant=0.00011472]"));
      ...
      onoff.SetAttribute ("PacketSize", UintegerValue (poissonPayloadSize));
      onoff.SetAttribute ("DataRate", DataRateValue (poissonDataRate)); //bit/s  
      ...
      std::cout<<"\nClient App "<<i<<" starts at "<<clientTempStartTime.GetMicroSeconds()<<" us\n";
  ...
  double expectedAppThroughputKbps = poissonPayloadSize*8.0*packetsPerSecond/(1.0e3);
  // std::cout<<"\nExpected APP datarate per STA (kbps)= "<<poissonPayloadSize*8.0*packetsPerSecond/(1.0e3)<<" kbps \n\n";
  std::cout<<"\nExpected APP datarate per STA (kbps)= "<<expectedAppThroughputKbps<<" kbps \n\n";
  // std::cout<<"\nExpected MAC datarate per STA (kbps)= "<<(poissonPayloadSize+66)*8.0*packetCountPerBeaconPeriod/(beaconInterval.GetMicroSeconds()/1.0e3)<<"\n\n";
  ...
  for (auto it = staMacToTotalBitsUplinkDownlink.begin (); it != staMacToTotalBitsUplinkDownlink.end (); it++)
  {
    ...
    <<";nSTA="<<nStations
    <<";traffic="<<"UDP_UL_only"
    <<";expected_UL_throughput_kbps="<<expectedAppThroughputKbps
  ...
  //   for (uint32_t i = 0; i < serverApp.GetN (); i++)
  //   {
  //     totalRxBytes[i] = poissonPayloadSize * DynamicCast<UdpServer> (serverApp.Get (i))->GetReceived ();
  //     totalRxBytes[i] = totalRxBytes[i] - rxBytesAt10Seconds[i];
  //     // std::cout<<"At UDP Server "<<i<<" Rx Bytes after t=10s is recorded as: "<<totalRxBytes[i]<<"\n";
  //     throughput[i] = (totalRxBytes[i] * 8) / ((simulationTime - 10.0) * 1000.0); //Kbit/s
  //     std::cout<<"app Goodput At Udp Server"<<i<<" (Kbps)="<<throughput[i]<<";\n";
  //     f1<<"log="<<currentLoopIndex_string <<";randSeed=" << randSeed<<";protocol="<<protocol<<";server="<<i+1<<";ExpectedthroughputKbps="<<expectedAppThroughputKbps<<";goodput="<<throughput[i]<<";avgCurrent_mA="<<avgCurrent_mAForSTA[i]<<";avgLatency_us="<<avgDelay_us[i]<< std::endl;
  //   } 
  // }
  // else if (protocol == "tcp")
  // {
  // for (uint32_t i = 0; i < serverApp.GetN (); i++)
  // {
  //   totalRxBytes[i] = DynamicCast<PacketSink> (serverApp.Get (i))->GetTotalRx ();
  //   totalRxBytes[i] = totalRxBytes[i] - rxBytesAt10Seconds[i];
  //   // std::cout<<"At TCP Server "<<i<<" Rx Bytes after t=10s is recorded as: "<<totalRxBytes[i]<<"\n";
  //   throughput[i] = (totalRxBytes[i] * 8) / ((simulationTime - 10.0) * 1000.0); //Kbit/s
  //   std::cout<<"app Goodput At Tcp Server "<<i<<"  (Kbps) ="<<throughput[i]<<";\n";
  //   // f1 <<"log="<<currentLoopIndex_string <<";randSeed=" << randSeed<<";protocol="<<protocol<<";server="<<i+1<<";ExpectedthroughputKbps="<<expectedAppThroughputKbps<<";goodput="<<throughput[i]<<";avgCurrent_mA = "<<avgCurrent_mAForSTA[i]<<";avgLatency_us="<<avgDelay_us[i]<< std::endl;
  // }
  // }
  ...
  std::cout<<std::setw(30) << std::right << "PS useCase" << ":\t" << useCase << std::endl;
  ...
  // std::cout<<std::setw(30)<< std::right << "Payload Size" << ":\t" <<poissonPayloadSize << " bytes" << std::endl;
