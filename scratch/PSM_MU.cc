...
void PhyStateTrace (std::string context, Time start, Time duration, WifiPhyState state)
{
  ...
  f << "state=" <<state<< "; startTime_s="<<start.GetSeconds()<<"; duration_s=" << duration.GetSeconds()<<"; nextStateShouldStartAt_s="<<(start + duration).GetSeconds()<<"; "<< std::endl;
}
...
main (int argc, char *argv[])
{
  ...
  // LogComponentEnable ("StaWifiMac", LogLevel (LOG_PREFIX_TIME | LOG_PREFIX_NODE | LOG_DEBUG));
...
// Enable / disable PSM and sleep state using MAC attribute change - through a function - not directly changing the MAC attribute 
  if (enablePSM_flag)
  {
    ...
    Simulator::Schedule (Seconds (8.5), &changeStaPSM, staMac, true);
  }
