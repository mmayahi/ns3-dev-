 ...
 * 
 * To run - example:
 *  ./ns3 --run "PSM_MU --simulationTime=5 --enablePSM_flag=true --enableUdpUplink=true --dataPeriod=0.33 --otherStaCount=10 --loopIndex=123321 --dataRatebps_other=31000 --enableMulticast=true --multicastInterval_ms=200" >>trial123.out 
 */
...
main (int argc, char *argv[])
{
  ...
  uint32_t otherStaCount = 2;  
  uint32_t dataRatebps_other = 1000;  
  ...
  cmd.AddValue ("enableUdpUplink", "Enable/Disable uplink UDP from PS STA. Set to true/false", enableUdpUplink);
  cmd.AddValue ("enablePSM_flag", "Enable/Disable PSM for the PS STA. Set to true/false", enablePSM_flag);
  cmd.AddValue ("dataPeriod", "Uplink UDP data rate from PS STA. Double value. Used only if enableUdpUplink is true.", dataPeriod);
...
//---------------------------------
  ...
  ApplicationContainer sinkApp = sinkHelper.Install (MainUDPServerNode);
  sink = StaticCast<PacketSink> (sinkApp.Get (0));
  sinkApp.Start (Seconds (0.0));

  /* Install UDP Transmitter on the station */ //Uncomment to introduce traffic
  if (enableUdpUplink)
  {

    UdpClientHelper UdpPSMClient (P2PInterfaces.GetAddress (1), 9); //PSM clients sent to this interface: 10.1.1.1
    UdpPSMClient.SetAttribute ("MaxPackets", UintegerValue (maxUdpPacketCount));
    UdpPSMClient.SetAttribute ("Interval", TimeValue (Seconds(dataPeriod)));
    UdpPSMClient.SetAttribute ("PacketSize", UintegerValue (payloadSize));
    ApplicationContainer UdpPSMClientApp = UdpPSMClient.Install (staWifiNode);
    
    UdpPSMClientApp.Start (Seconds (7.0 + ClientAppRand->GetValue()));

    // OnOffHelper server ("ns3::UdpSocketFactory", (InetSocketAddress (P2PInterfaces.GetAddress (1), 9)));
  ...
  for (uint32_t ii = 0; ii < otherStaCount; ii++)
  {
    ...
    UdpServerHelper server ((100 + ii)); //port number
    ...
    //other clients sent to this interface: 10.0.0.1
    UdpClientHelper tempClient (apInterface.GetAddress (0), (100 + ii)); //ip4 address (of the remote UDP server), port number.
